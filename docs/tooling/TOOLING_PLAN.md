...
- **Branch: `chore/release-tooling-foundation`** (separate from the current `chore/release-tooling` which carries item #1).
...
After the foundation PR merges, the items below can land independently.
...
**Status:** Shipped in PR #6 *(without the `pnpm build && pnpm test` pre-publish guard — see #6 + foundation)*. Workflow file name differs from earlier draft (`release.yml`).
...
2. User confirms.
3. Implement, test, commit on a per-item branch (e.g., `chore/playwright-e2e`, `chore/jira-mirror`); each item ships as its own PR.
4. Move to next item.

**Standalone items (no foundation needed):** #1 (shipped in PR #6), #5, #11, #12, #14, #18, #19, #20, #22.

**Needs foundation:** all others. The foundation lands as a single upfront PR on `chore/release-tooling-foundation` per the Foundation section above; the package-manager and delivery-shape decisions are already locked in (pnpm; one upfront PR).
